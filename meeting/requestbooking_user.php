...
// ดึงค่าที่จองออกมาให้หมด
	$sql = "select ev_id from event where event_id = '$event_id'";
	...
	if($chk >0){
		...
		echo "<script>Swal.fire({
			...
			showConfirmButton: true,
				...
				window.location = 'showrequestbooking_user.php
				});
				</script>";
				return; 
			}else{
...
if ($num >= 0 && $sing == "+") {  		// ส่วนของ datediff
	...
	if ($rows > 0) {
		...
		$sql1 = "ALTER TABLE event AUTO_INCREMENT = 1";
...
for ($i = 0; $i <= $num; $i++) {	// ส่วนของ datediff INSERT event_id ตัวแปร $event_id
	...
	// echo "<br>";
	include "connect.php";
	// $sql = "INSERT INTO event(ev_startdate) VALUES ('$ev_startdate')";
	// $query=  mysqli_query($con, $sql) or die(mysqli_error(conn));
	$sql1 ="INSERT INTO event(ev_title,ev_people,ro_id,st_id,id,ev_startdate,ev_enddate,ev_starttime,ev_endtime,ev_status,event_id) 
	VALUES ('$ev_title','$ev_people','$ro_id','$st_id','$id','$dateStart','$ev_enddate','$ev_starttime','$ev_endtime','$ev_status','$event_id')";
	$query1=  mysqli_query($con, $sql1) or die(mysqli_error(conn));
...
echo "<script>Swal.fire({
	...
	showConfirmButton: true,
		...
		window.location = 'showrequest_user.php'
		});
