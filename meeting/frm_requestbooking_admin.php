...
if (!$_SESSION['userid']) {
  ...
  from event as event
  inner join rooms as rooms on event.ro_id = rooms.ro_id 
  inner join style as style on event.st_id = style.st_id
  inner join member as member on event.id = member.id
  inner join department as department on member.de_id = department.de_id where ev_id='$ev_id'";
  ...
  <body id="page-top">
   ...
   <div class="col-sm-12">
     <div class="wrap-form">
      <center>
        <h4 class="text-dark" ><i class="far fa-edit"></i>  อนุมัติแบบฟอร์มขออนุญาตใช้ห้องประชุม</h4>

      </center>
      <br>
      <form action="requestbooking_admin.php" method="post" accept-charset="utf-8">
        ...
        <input name="room_id" type="hidden" id="room_id" value="<?php echo "$rs[ro_id]"; ?>" />
